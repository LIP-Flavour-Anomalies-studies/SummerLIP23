...
#Define signal model (Gaussian + CB) and its parameters
#Gaussian model
...
#Number of bins for the histogram B-candidate mass
nbins=100
...
#Sideband edges for 3sigma
sideband_edge1 = Mean - 3 * Sigma
sideband_edge2 = Mean + 3 * Sigma
...
for bin in range(1, nbins+1):
    ...
    if bin_edge_high <= sideband_edge1:
       bkg_count1+=bin_count
    elif bin_edge_low >= sideband_edge2:
       bkg_count2+=bin_count

#Sidebands info
print(f"Left Sideband: {bkg_count1} events,   edge: {sideband_edge1} GeV \n")
print(f"Right Sideband: {bkg_count2} events,   edge: {sideband_edge2} GeV \n")


#Background under the signal
bkg_count3 = n_back.getVal() - bkg_count1 - bkg_count2
print(f"Background under signal: {bkg_count3} events")

#integrate and compare it for sidebands and bkg_count3


#Background scalling factor
fb = bkg_count3 / (bkg_count1+bkg_count2)
...
l1 = ROOT.TLine(sideband_edge1, 0., sideband_edge1, hmass.GetBinContent(binNumber1)+200)
l2 = ROOT.TLine(sideband_edge2, 0., sideband_edge2, hmass.GetBinContent(binNumber1)+200)
...
L.DrawLatex(0.15,0.70,ROOT.Form("coeff_{1}: %.3f #pm %.3f" % (sig1_frac.getVal(),sig1_frac.getError())))
...
#Write *.txt file
f = open("scalling_fac.txt", "w")
f.write(f"""Left sideband edge: {sideband_edge1} GeV \n
Right sideband edge: {sideband_edge2} GeV \n \n
MC events: {mcSignalEvents} events \n \n
Background scalling factor, fb = {fb} \n
Signal scalling factor, fs = {fs} \n""")
f.close()
